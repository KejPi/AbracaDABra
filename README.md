...
RTL-SDR and Airspy devices need WinUSB driver for correct functionality under Windows. To install it, use Zadig that can be downloaded from [release page](https://github.com/KejPi/AbracaDABra/releases/latest). If you have not installed it before, please follow installation steps described [zadig.akeo.ie](https://zadig.akeo.ie). When the driver is installed, start AbracaDABra.exe and first band scan should start automatically when RTL-SDR device is recognized. 
