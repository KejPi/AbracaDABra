...
There is also a commandline interface (CLI) available for listening to radio from the commandline or for streaming over a web UI.
A detailed description of available options and settings can be found in the help menu:
```
AbracaDABra --cli --help
```

By default terminal UI is displayed:
```
AbracaDABra --cli
```

By handing over a port (and bind adress) you can enable the web UI you can open in a web browser:
```
AbracaDABra --cli -p 8080 --bind 192.168.178.13
```
...
_Note:_ There are two binaries distributed for Windows, use `AbracaDABra.com` to work with CLI under Windows. The other possibility is to start CLI using `AbracaDABra.exe` this way: 
```
start /wait "" AbracaDABra.exe --cli
```
